...
summary_values_tbl <- reactive({
  ...
  summarize(
  # counts number of unique orders.
  health_metric = unique(order.id) %>% length(),  
  wealth_metric = sum(extended_price),
  wise_metric = round(sum(str_detect(category_1,"Mountain"))/ (sum(str_detect(category_1,"Road")+0.001)),1)
) %>% 
  mutate(
    health_metric = health_metric %>%
      scales::number(big.mark = ","),
    wealth_metric = wealth_metric %>%  
      scales::dollar(scale = 1E-6,accuracy = 0.1, suffix   = "M")
         )
})
