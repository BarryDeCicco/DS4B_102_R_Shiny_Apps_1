...
shinyWidgets::radioGroupButtons(
  ...
  selected = "month",
  status   = "primary",
  # status = "success" would give greens, "danger" reds.
  justified = TRUE,
  checkIcon = list( 
    yes = icon("ok", lib="glyphicon"),
    no  = NULL
...
# this produces a data frame based on the date range picker:

time_plot_tbl <- reactive({
  
    time_unit = "month"  # this controls the aggregation below.
    ...
    filter(category_1 %in% input$checkbox_category_1) %>% 
    filter(category_2 %in% input$picker_category_2) %>% 
      # floor is from lubridate - it sets dates to 'yyyy-mm-01'.
...
#
# renderDataTable({
#   time_plot_tbl()    # REMEMBER THE (), OR NOTHING WILL PRINT!!!
#   })
...
output$plotly_2  <- renderPlotly({
  g <- time_plot_tbl() %>%
    ...
    geom_point(aes(text = label_text),
               color = "#2c3e50", size = 0.1) +
    geom_smooth(method = "loess", span = 0.2)     +
  
    theme_tq()    +          # adds tidyquant theme
    expand_limits(y=0) +
