...
```
Row {data-heighth=150}
---------------------------------------------------------------


```{r}

# the 'fa' is for Font Awesome.


summary_values_tbl <- reactive({
  processed_data_filtered_tbl() %>% summarize(
  health_metric = unique(order.id) %>% length(),  # counts number of unique orders.
  wealth_metric = sum(extended_price),
  wise_metrict = sum(str_detect(category_1,"Mountain"))/(sum(str_detect(category_1,"Road")+0.001)) %>%  # last is to avoid DIV ZERO errors.
    round(1)
) %>% 
  mutate(
