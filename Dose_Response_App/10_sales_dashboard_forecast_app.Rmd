...
# dose_response_df <- read.csv("Dose_Response_App/data.csv", header = TRUE)

# When running as shiny:
dose_response_df <- read.csv("data.csv", header = TRUE)

# View(dose_response_df)
# class(dose_response_df)
# names(dose_response_df)
# sapply(dose_response_df, class) 
...
dose_response_df$Drug = as.character(dose_response_df$Drug)
dose_response_df$Drug = tolower(dose_response_df$Drug)
DRUG.NAME = data.frame(unique(dose_response_df$Drug))
Drugs_cov = DRUG.NAME
names(Drugs_cov) = c("DRUG.NAME")
Drugs_cov$DRUG.NAME = as.character(Drugs_cov$DRUG.NAME)

for (name in Drugs_cov$DRUG.NAME){
     data = subset(dose_response_df, Drug == name)
#	   print(summary(aov(lm(dose_response_df$Response ~ dose_response_df$CONCEN.uM))))
	   Drugs_cov$pvalue[Drugs_cov$DRUG.NAME == name] = 
	       unlist(summary(aov(lm(dose_response_df$Response ~ dose_response_df$CONCEN.uM))))[9]} 
...
```{r dose-response-workshop-selection-controls}
...
shinyWidgets::pickerInput(
  ...
  selected = unique(Drugs_cov$DRUG.NAME) %>% sort(), 
  multiple = TRUE,
  options  = list(
    `actions-box` = TRUE,
    size = 10,
...
```{r dose-response-workshop-filter-dose-response-data-set}
processed_data_filtered_tbl <- eventReactive(
  eventExpr = input$apply, 
...
# add an observeEvent() function, so that whenever the forecast mode is 'TRUE',
# clicking on the Time Unit buttons above the plot will immediately change
# the plot without having to hit the 'apply' button. 
# Also add a delay, to allow time for the screen to update.
...
# add an observeEvent() function, so that whenever the forecast mode changed',
# the plot will be changed without having to hit the 'apply' button. 
# Also add a delay, to allow time for the screen to update.
