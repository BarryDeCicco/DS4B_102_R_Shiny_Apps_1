...
```{r dose-response-workshop-filter-dose-response-data-set, eval=FALSE}
data <- eventReactive(
  eventExpr = input$apply, 
...
```{r dose-response-workshop-filter-dose-response-data-set-II, eval=FALSE}
data <- eventReactive(
  eventExpr = input$apply, 
                                             
  valueExpr = {
  
Data %>% filter(Drug == input$picker_drug_name)
  },
  ignoreNULL = FALSE
)
...
```{R dose-response-workshop-error-bar-figure, eval=FALSE}

error_bar_plot <- eventReactive(
  eventExpr = input$apply, 
                                             
  valueExpr = {
  
    dataagg <- Data %>% filter(Drug == input$picker_drug_name) %>% 
        cbind(aggregate(data$Response, by = list(data$CONCEN.uM), FUN = mean, na.rm = T), 
              aggregate(data$Response, by = list(data$CONCEN.uM), FUN = sd, na.rm = T)$x)
