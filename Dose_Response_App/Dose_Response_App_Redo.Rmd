...
```{r dose-response-workshop-select-model, eval=TRUE}
...
fct_command <- eventReactive(
  ...
  valueExpr = {
      ...
      fct_command <- Model_List %>% 
    dplyr::filter(Model_Name %in% input$picker_model_name) %>% 
    select(fct_command)
  },
  ignoreNULL = FALSE
)
...
```{r create-model-name-reactive-object, eval=TRUE}
model_name_reactive() <- reactive({
  
    input$picker_model_name
    
})

renderPrint(model_name_reactive())
