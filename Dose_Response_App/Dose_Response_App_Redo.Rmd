...
```{r run-model-selected, eval = FALSE}



model_fit <- reactive({
    
   model <- case_when( input$picker_model_name == "3-parameter log-logistic model" ~ 
       drm(Response ~ CONCEN.uM, data = na.omit(data), fct = LL.3u(upper = 100)),
                          input$picker_model_name == "4-parameter log-logistic model" ~ 
           drm(Response ~ CONCEN.uM, data = na.omit(data), fct = LL.4())
   )
