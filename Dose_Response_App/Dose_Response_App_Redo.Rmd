...
model_fit <- reactive({
    ...
    model <- drm(Response ~ CONCEN.uM, data = na.omit(scatterplot_tbl()), fct = LL.3u(upper = 100))
    

})

renderPrint(class(model_fit()))

```



```{r set-up-model-statement, eval=FALSE}


model_statement <- reactive({
  
    
   statement <- drm(Response ~ CONCEN.uM, data = na.omit(dataagg_tbl()),
                    fct = model_row_reactive()$Model_Command)

})

# renderPrint(model_statement())

```


```{r run-model-old, eval=FALSE}


model_fit <- reactive({
  
    
   model_fit <- drm(Response ~ CONCEN.uM, data = na.omit(dataagg_tbl()),
                    fct = renderPrint(model_row_reactive()$Model_Command))
...
scatterplot_title = paste0("Dose and Response:  Drug = ", input$picker_drug_name, 
                           "\n Model = ",model_row_reactive()$Model_Name)
