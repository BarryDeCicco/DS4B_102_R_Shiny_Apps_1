...
category_hierarchy_tbl <- bikes_tbl %>% separate_bike_description(
...
# Bike Type (Category 1) ----       Determined from category 2 value.
h4("Bike Type")
bike_type <- output$bike_type <- reactive({
...
textOutput(outputId = "bike_type") %>% em() %>% strong() %>% h1()
...
# This resets the text input box:  
  updateTextInput(session = session, 
                  inputId = "text_bike_model", 
                  value = "Jekyll Al 1 Black Inc.")

# This resets the picker button (bike family selector dropdown):
