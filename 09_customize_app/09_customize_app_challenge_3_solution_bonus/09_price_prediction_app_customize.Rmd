...
library(tidyverse)
library(tidyquant)
library(fs)                # for file system commands.
...
# 2.0 Data ----
# This is for running the app from shiny:
con <- dbConnect(RSQLite::SQLite(), "../../00_data/bikes_database.db")

# This is for running the app as an ordinary Rmd or R script:
# con <- dbConnect(RSQLite::SQLite(), "00_data/bikes_database.db")
...
# CSS causes renderPrint() to print in capitals - the data is not!
# 1. Render lower case; 2.  Replace spaces with underscores.

# First, run "generate_new_bike("F-Si","Mountain","Cross Country Race","Aluminum", .ml_model = model_xgboost)" to get new data.

new_bike_tbl <- generate_new_bike("Beast of the East","Mountain","Cross Country Race","Aluminum", 
                                  .ml_model = model_xgboost)
...
renderPrint({
  ...
  directory_files <- fs::dir_ls("09_customize_app/09_customize_app_challenge_3_solution_bonus/bike_img")
  detection_result <- directory_files %>% 
    str_detect(pattern = str_glue("bike_img/{detect}"))
  
  path <- directory_files[detection_result]
